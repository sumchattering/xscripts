...
# Created By Sumeru Chatterjee on 09/14/11
...
def main():
	usage = '''%prog -p <Project Directory> -d <Resource Directory> <options>
...
The source files that are searched are (*.h,*.m,*.xib,*.nib)'''
	...
	if not((options.__dict__["resource_dir"]) and (options.__dict__["project_dir"])):
		parser.print_help()
		exit(0)
	...
	if resourcelist:
		...
		if verbose:
			...
			print "-----------------\n"
			print "\n".join(os.path.basename(file) for file in resourcelist)
			print "\n\n"
	else:
	...
	for resource_file in resourcelist:
                ...
                if not valid:
            if not valid:
valid_resource_list.append(resource_file)
                        print "Resource %s - Invalid"%os.path.basename(resource_file)
                else:
                        if verbose:
                                print "Resource %s - Valid"%os.path.basename(resource_file)
...
"Resource %s - Valid"%os.path.basename(resource_file)

if verbose:

        if move and invalid_resource_list:
"Resource %s - Valid"%os.pa
                if not os.path.exists(invalid_dir):
                        os.makedirs(invalid_dir)

ource_list:
"Resource %s                 for resource_file in invalid_resource_list:
                        src = resource_file
...
def validate(resource_file,sourcelist_file,verbose):
	if os.path.exists(resource_file):
		...
		ext = "".join(resourcefilename.split('.')[1:])
                ...
                regex = "(<string.*?>|\")%(name)s(.%(ext)s)?(</string>|\")"%{'name':filename,'ext':ext}
...
def get_resource_files(resource_dir,norecurse=False):
	...
	if os.path.exists(resource_dir):
		if not norecurse:
         ...
         for file in p1.stdout:
DS_Store" in file and not "INVALID_RESOURCES/" in file:
